...
int main(int argc, char *argv[])
{
    ...
    auto sphere2 = Sphere(Point3f(0, 0, 0), 1);
    ...
    // Loads latest file
    int highestId = getNewHdrBmpId("./boxRenders") - 1;
    if (highestId >= 0) {
        std::cout << "Loading image " << highestId << std::endl;
        camera->film->loadContributionsFromFile("./boxRenders/box_" + std::to_string(highestId));
    }
    ...
    for (int i = 0; i < N_RENDERS; i++)
    {
        ...
        int newId = getNewHdrBmpId("./boxRenders");

        //film.filterIndirectLight();
        //film.loadContributionsFromFile("test");
        film->storeContributionsToFile("./boxRenders/box_" + std::to_string(newId));
        std::cout << "Stored image " << newId << std::endl;

        film->writeToBMP("./boxRenders/box_" + std::to_string(newId), ToneMappingType::gammaClamp);
    }
    
    camera->film->writeToBMP("box", ToneMappingType::gammaClamp);
}