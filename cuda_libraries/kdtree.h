            ...
            int shape_id=-1) const 
    {
        if (right > left) 
        {
            ...
            if (norm(difference(p,elements[median]))<max_distance 
                    && (shape_id == -1 || elements[median].shapeId == shape_id)) 
            {
            ...
            if ((right-left)>1) 
            {
                ...
                {//First left node and then, if needed, right node
                    nearest_neighbors_impl(values,left,median,p,number,max_distance,norm, shape_id);
                    if (norm(difference(p,pplane)) < max_distance) //We still need to explore the other node
                        nearest_neighbors_impl(values,median+1,right,p,number,max_distance,norm, shape_id);
...
public:
    ...
    {
        std::vector<T> sol;
        nearest_neighbors_impl(sol,0,elements.size(),p,number,max_distance,norm, shape_id);
        return sol;
    }
    ...
    std::vector<T> nearest_neighbors(const P& p, std::size_t number, 
        ...
        return nearest_neighbors(p_impl,number,max_distance,norm, shape_id);
    }
